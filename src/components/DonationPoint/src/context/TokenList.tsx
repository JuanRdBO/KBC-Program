...
type TokenListContext = {
  ...
  tokens: TokenInfo[];
  tokensSollet: TokenInfo[];
  tokensWormhole: TokenInfo[];
};
...
export function TokenListContextProvider(props: any) {
  ...
  const [tokensWormhole, wormholeMap] = useMemo(() => {
    const tokens = tokenList.filter((t: TokenInfo) => {
...
export function useTokens() {
  const { tokens, tokensWormhole, tokensSollet } =
    useTokenListContext();
  return { tokens, tokensWormhole, tokensSollet };
}
